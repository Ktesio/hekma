...
use hekma_adapter_api::MeteringSource;
use hekma_engine::{broadcast, EngineEvent, EventSubscription};
...
///   ([`hekma_adapter_api::CONTRACT_VERSION`]) — never a per-suite literal.
...
impl ManifestFixture {
    ...
    pub fn write(self, dir: &Path) -> PathBuf {
        ...
        let body = format!(
            ...
            contract_version = hekma_adapter_api::CONTRACT_VERSION,
            kind = self.kind,
            exec = exec,
            args_toml = args_toml,
...
/// `cargo build -p hekma-conformance --bin fake_agent` so the process-spawning
...
/// hekma-conformance --bin fake_agent` before the suite) and the
...
mod tests {
    ...
    fn frozen_constants_agree_between_the_builder_and_uj3() {
        ...
        assert_eq!(
            hekma_adapter_api::CONTRACT_VERSION,
    ...
    fn drain_subscription_drains_while_alive_and_panics_on_closed() {
        ...
        let dir = TempDir::new().unwrap();
        let engine =
            hekma_engine::Engine::open(Some(dir.path().to_path_buf())).expect("open engine");
