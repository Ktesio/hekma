...
//!
//! Story 7-1's intent (FR-31): a host links `hekma-engine` and drives the full
...
pub fn committed_state(state_dir: &Path, name: &str) -> Option<String> {
    let conn =
        rusqlite::Connection::open(state_dir.join(hekma_engine::paths::STATE_DB_FILE)).ok()?;
...
pub fn stop_resilient(facade: &::hekma_engine::Blocking<'_>, name: &str, window: Duration) {
...
pub fn stop_all_resilient(facade: &::hekma_engine::Blocking<'_>, names: &[&str], window: Duration) {
    for name in names {
