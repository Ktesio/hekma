//! # hemaka-adapters-hermes
//!
//! Native (in-workspace Rust) adapter for the NousResearch [Hermes Agent],
//! implementing the Adapter Contract from `hemaka-adapter-api` (architecture
...
/// across the adapter boundary (AD-2 keeps `hemaka-adapters-hermes` depending
/// only on `hemaka-adapter-api`), so the adapter declares its own literal copy.
