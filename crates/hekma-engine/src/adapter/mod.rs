...
//!
//! The loader reads, parses (via [`hekma_adapter_api::Manifest`]), and
...
//! `hekma-adapter-api`'s parsed [`Manifest`]/[`CapabilityDeclaration`]/
...
//! [`builtin::BuiltinMock`], deliberately NOT the `hekma-conformance`
...
pub enum AdapterResolveError {
    ...
    /// [`hekma_adapter_api::ContractVersionError`], so the rule text lives in
...
/// `[lifecycle.start]` [`OpTemplate`](hekma_adapter_api::OpTemplate) or a native
...
pub enum LaunchResolveError {
    ...
    /// [`hekma_adapter_api::ContractVersionError`]).
    #[error("adapter.toml at {path} is incompatible: {detail}")]
    ContractIncompatible {
        /// The manifest path.
        path: String,
...
///   (reusing [`hekma_adapter_api::Manifest`] — the same parser registration
...
pub fn resolve_start_launch(
    ...
    let manifest_version = manifest
        ...
        .unwrap_or_default()
        .trim();
    if let Err(negotiation) = hekma_adapter_api::negotiate_contract_version(manifest_version) {
        ...
        for version in ["2.1.0", "2.0.0-rc.1", "2.0.0+build.9"] {
            ...
            match err {
                AdapterResolveError::ContractIncompatible { path, detail } => {
                    ...
                    assert!(
                        detail.contains(hekma_adapter_api::CONTRACT_VERSION),
                        "names the engine version: {detail}"
                    );
                    assert!(
                        detail.contains(hekma_adapter_api::COMPATIBILITY_RULE),
...
"#;
    ...
    fn enforce_invariants_rejects_all_unsupported_native_declaration() {
        ...
        use hekma_adapter_api::{Capability, SupportLevel};
...
"#;
    ...
    fn resolve_start_launch_resolves_the_hermes_builtin_launch() {
        ...
        let launch = resolve_start_launch(hekma_adapters_hermes::HERMES_KIND, None)
...
"#;
        let path = write_manifest(tmp.path(), body);
        let launch = resolve_start_launch(hekma_adapters_hermes::HERMES_KIND, Some(&path))
...
"#;
    ...
    fn apply_delivers_resolved_cleartext_for_a_secret_leaf_not_the_mask() {
        ...
        assert_ne!(
            launch.env.get("MODEL").map(String::as_str),
            Some(hekma_adapter_api::OsId::current().as_str()), // arbitrary non-equal
        );
        assert_ne!(
            launch.env.get("MODEL").map(String::as_str),
    ...
    fn hekma_engine_secret_mask() -> &'static str {
...
"#;
    ...
    // `crates/hekma-engine/tests/atomic_config_writes.rs`
