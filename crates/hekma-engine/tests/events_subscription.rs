...
//!    Driven with the SHARED `hekma_conformance::uj3` fixture (the same
...
use hekma_conformance::test_support::{self, ManifestFixture};
use hekma_conformance::uj3::{self, committed_usage_rows, usage_from_payload, CommittedUsage};
use hekma_engine::{
...
//
// The fixture manifests are the SHARED `hekma_conformance::test_support`
...
fn wait_until_state(
    ...
    name: &str,
    pred: impl Fn(hekma_engine::LifecycleState) -> bool,
    within: Duration,
    what: &str,
) -> hekma_engine::LifecycleState {
    let deadline = Instant::now() + within;
    loop {
        let state = facade
            .instance_status(name)
            .map(|s| s.instance.state)
            .unwrap_or(hekma_engine::LifecycleState::Registered);
        if pred(state) {
            return state;
        }
        assert!(
...
fn barrier(facade: &hekma_engine::Blocking<'_>) {
...
fn committed_transitions(facade: &hekma_engine::Blocking<'_>, name: &str) -> Vec<TransitionEvent> {
    facade
...
fn usage_of(events: &[EngineEvent]) -> Vec<hekma_engine::UsageUpdateEvent> {
    events
        .iter()
        .filter_map(|e| match e {
...
// `hekma_conformance::uj3` so this suite and the 7-3 collision test compare
...
fn usage_row_count(state_dir: &Path, name: &str) -> u64 {
    let conn = rusqlite::Connection::open(state_dir.join(hekma_engine::paths::STATE_DB_FILE))
...
fn assert_usage_matches_rows(
    usage: &[hekma_engine::UsageUpdateEvent],
    rows: &[CommittedUsage],
    what: &str,
) {
    assert_eq!(
...
fn subscriber_sees_the_uj3_flow_in_commit_order_with_schema_valid_payloads() {
    ...
    uj3::wait_for_state(
        state.path(),
        uj3::FLOW_INSTANCE,
        hekma_engine::LifecycleState::Paused,
    ...
    let breaches: Vec<hekma_engine::BudgetBreachEvent> = events
        .iter()
        .filter_map(|e| match e {
    ...
    match &phase1[first_breach + 1] {
        EngineEvent::Transition(t) => {
            assert_eq!(t.new_state, hekma_engine::LifecycleState::Paused);
            assert!(
...
fn subscriber_sees_the_crashed_and_restarted_transitions() {
    ...
    wait_until_state(
        &facade,
        "sub-crashy",
        |s| s == hekma_engine::LifecycleState::Running,
...
fn stalled_subscriber_never_stalls_supervision_and_observes_lagged() {
    ...
    assert_eq!(
        ...
        hekma_engine::LifecycleState::Running,
...
fn interleaved_instances_keep_per_instance_fifo() {
    ...
    let expected: Vec<(
        ...
        (
            "fifo-a",
            hekma_engine::LifecycleState::Registered,
            hekma_engine::LifecycleState::Starting,
        ),
        (
            "fifo-a",
            hekma_engine::LifecycleState::Starting,
...
fn a_failed_transition_append_publishes_nothing_and_the_next_commit_delivers() {
    ...
    assert!(
        transitions_of(&baseline)
            .iter()
            .all(|t| t.new_state != hekma_engine::LifecycleState::Paused),
...
fn a_failed_breach_append_still_enforces_but_publishes_no_breach() {
    ...
    assert!(
        committed
            .iter()
            .any(|e| e.new_state == hekma_engine::LifecycleState::Paused),
