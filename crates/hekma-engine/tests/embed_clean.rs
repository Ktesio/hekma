...
//!    SHARED `hekma_conformance::uj3` expectations (same expectations,
...
//!    `crates/hekma/Cargo.toml` declares no tokio edge. The full build-level
...
use hekma_engine::{broadcast, AdapterRef, ConfigLayer, Engine, EngineEvent, LifecycleState};
...
struct FlowResult {
    ...
    /// [`EVENT_BUS_CAPACITY`](hekma_engine::EVENT_BUS_CAPACITY)).
    lagged: Option<u64>,
}
...
fn two_engines_in_one_process_drive_the_full_uj3_flow_concurrently_without_collision() {
    ...
    let _warm = hekma_conformance::fake_agent_bin();
...
fn assert_subscriber_isolation(result: &FlowResult) {
    ...
    let received: Vec<&hekma_engine::TransitionEvent> = result
        .events
        .iter()
    ...
    let received_breaches: Vec<&hekma_engine::BudgetBreachEvent> = result
        .events
        .iter()
        .filter_map(|e| match e {
...
fn instance_row_count(root: &Path) -> u64 {
    let conn = rusqlite::Connection::open(root.join(hekma_engine::paths::STATE_DB_FILE))
...
fn fleet_entry(facade: &hekma_engine::Blocking<'_>) -> hekma_engine::FleetEntry {
    facade
        .fleet()
        .expect("fleet read")
        .into_iter()
...
fn kt_consumes_the_engine_only_through_the_blocking_facade() {
    let kt_src = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../hekma/src")
        .canonicalize()
        .expect("the hekma CLI's src tree sits beside the engine crate in the workspace");
    ...
    let manifest =
        std::fs::read_to_string(kt_src.parent().expect("kt crate root").join("Cargo.toml"))
            .expect("read the hekma CLI Cargo.toml");
