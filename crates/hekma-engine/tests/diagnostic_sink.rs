...
//! [`DiagnosticSink`](hekma_engine::DiagnosticSink) when one is installed
...
//!   asserts the child's captured stderr carries exactly the two `[hekma]`
...
//!   child's stderr contains no `[hekma]` line at all while the sink's
...
use hekma_conformance::test_support::ManifestFixture;
...
/// (the choke point emits `[hekma] ` + this text + a terminating '\n').
/// Shared by the byte-exact suites and the rotation/Arc-sharing suites.
fn expected_notice_line(dir: &Path) -> String {
    format!(
        "[hekma] {NOTICE_INSTANCE}: a 'filesystem' Memory Backing is attached (managed \
...
fn expected_breadcrumb_line(pause_err: &EngineError) -> String {
    format!("[hekma] {BREADCRUMB_INSTANCE}: budget breach pause could not be honored: {pause_err}")
}
...
fn without_a_sink_both_diagnostics_reach_stderr_byte_identical() {
    ...
    // the child's stderr carries EXACTLY the two `[hekma]` diagnostic lines —
...
fn with_a_sink_installed_stderr_stays_silent() {
    ...
    // asserts NO `[hekma]` line reached the child's stderr while the relayed
