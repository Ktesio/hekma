...
//! gate's integration-test allowlist (`crates/hekma-engine/tests/`, the AI-35
...
use hekma_adapter_api::{ConfigMapping, ConfigTarget};
use hekma_engine::paths::write_atomically;
use hekma_engine::{ConfigLayer, Engine, SourceLayer};
...
/// An [`hekma_engine::EffectiveConfig`] whose single INSTANCE-layer leaf is
...
fn effective_with_model(value: &str) -> hekma_engine::EffectiveConfig {
    let layers = [
        ConfigLayer::empty(),
        ConfigLayer::empty(),
        ConfigLayer::parse(
    ...
    hekma_engine::domain::resolve(layers)
}
...
fn apply_file_target_failed_write_leaves_the_previous_native_file_unchanged() {
    ...
    let err = hekma_engine::adapter::apply_config_mapping(
        &mut launch,
        &mapping,
        &second,
        &BTreeMap::new(),
    ...
    assert!(
        matches!(
            err,
            hekma_engine::adapter::ConfigApplyError::FileRender { .. }
        ),
