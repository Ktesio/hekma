...
//! `hekma-engine`'s public API drives the FULL UJ-3 flow
...
//! `hekma_conformance::uj3` — the same module the CLI journey test
...
//! `kt` itself drives. `hekma_conformance` is used as TEST INFRASTRUCTURE
...
use hekma_engine::{AdapterRef, ConfigLayer, Engine, LifecycleState};
