...
fn the_same_attach_sequence_works_on_the_mock_and_a_manifest_adapter() {
    ...
    for leg in legs {
        ...
        if let Some(dump) = &leg.dump {
            ...
            let mapping = hekma_engine::adapter::resolve_config_mapping("mock", None).unwrap();
...
fn a_copied_agent_home_serves_a_byte_identical_memory_tree_and_reports_the_backing() {
    ...
    assert_eq!(
        fs_status.guarantee,
        hekma_engine::GuaranteeLevel::ManagedDirByteDurable
    );
    ...
    assert_eq!(
        ...
        hekma_engine::GuaranteeLevel::HomePersistenceOnly
    );
    assert!(
