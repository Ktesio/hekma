...
//! `hemaka-conformance` ALONE (this crate's own manifest adapter is the
...
use hemaka_conformance::{run_mock_conformance, section_ids, SectionResult};
...
fn write_adapter(dir: &Path) {
    let exec = hemaka_conformance::fake_agent_bin();
    let body = format!(
        r#"
...
fn third_party_manifest_adapter_is_proven_conformant_by_the_public_harness() {
    ...
    if hemaka_adapter_api::OsId::current() == hemaka_adapter_api::OsId::Other {
