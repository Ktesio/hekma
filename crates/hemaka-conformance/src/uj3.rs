...
//!
//! Story 7-1's intent (FR-31): a host links `hemaka-engine` and drives the full
...
pub fn committed_state(state_dir: &Path, name: &str) -> Option<String> {
    let conn =
        rusqlite::Connection::open(state_dir.join(hemaka_engine::paths::STATE_DB_FILE)).ok()?;
...
pub fn stop_resilient(facade: &::hemaka_engine::Blocking<'_>, name: &str, window: Duration) {
