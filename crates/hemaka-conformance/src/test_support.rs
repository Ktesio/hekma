...
use hemaka_adapter_api::MeteringSource;
use hemaka_engine::{broadcast, EngineEvent, EventSubscription};
...
///   ([`hemaka_adapter_api::CONTRACT_VERSION`]) — never a per-suite literal.
...
impl ManifestFixture {
    ...
    pub fn write(self, dir: &Path) -> PathBuf {
        ...
        let body = format!(
            ...
            contract_version = hemaka_adapter_api::CONTRACT_VERSION,
            kind = self.kind,
            exec = exec,
            args_toml = args_toml,
...
pub fn fake_agent_bin_in(hop: BinDir) -> PathBuf {
    ...
    #[cfg(not(tarpaulin_include))]
    {
        ...
        build
            .args(["build", "-p", "hemaka-conformance", "--bin", "fake_agent"])
        ...
        if !matches!(status, Ok(s) if s.success() && candidate.exists()) {
            panic!(
                "fake_agent binary not found at {} and an on-demand build did not produce it \
                 (build status: {status:?}). Build `hemaka-conformance` first.",
                candidate.display()
            );
        }
    }
...
/// `cargo build -p hemaka-conformance --bin fake_agent` so the process-spawning
...
/// hemaka-conformance --bin fake_agent` before the suite) and the
...
mod tests {
    ...
    fn drain_subscription_drains_while_alive_and_panics_on_closed() {
        ...
        let dir = TempDir::new().unwrap();
        let engine =
            hemaka_engine::Engine::open(Some(dir.path().to_path_buf())).expect("open engine");
