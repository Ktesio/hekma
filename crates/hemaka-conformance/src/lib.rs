...
//!   helpers the LIBRARY host test (`hemaka-engine`'s `tests/
...
//! public harness needs a live engine, so this crate takes `hemaka-engine` as
...
/// only on `hemaka-adapter-api` (never on the engine); the cross-boundary parity
