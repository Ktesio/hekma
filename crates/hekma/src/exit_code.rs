//! The `hekma` numeric exit-code contract (story 4-3, FR-26 / PRD §7).
//!
//! `hekma` returns a documented, stable set of numeric exit codes so it is
...
//! (`crates/hekma/tests/agent_cli.rs` + this module's own unit tests) pin them so an
...
//! `Box<dyn std::error::Error>` (miette lives in `hekma` only — conventions). Rather
