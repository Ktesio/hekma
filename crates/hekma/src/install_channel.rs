    ...
    P: CommandProbe,
{
    ...
    if components
        .windows(2)
        .any(|window| window[0] == "Cellar" && window[1] == "hekma")
    {
        return InstallChannel::Homebrew;
    }

    if command_probe.command_exists("brew")
        && (command_probe.command_succeeds("brew", &["list", "--formula", "hekma"])
            || command_probe.command_succeeds("brew", &["list", "--formula", "ktesio/tap/hekma"]))
    {
        return InstallChannel::Homebrew;
    }
...
mod tests {
    ...
    fn test_detect_install_channel_uses_homebrew_formula_probe() {
        let probe = FakeProbe::default()
            .with_command("brew")
            .with_success("brew", &["list", "--formula", "ktesio/tap/hekma"]);
        let channel =
            detect_install_channel_with_env(Path::new("/usr/local/bin/hekma"), None, None, &probe);
    ...
    fn test_detect_install_channel_wrapper_falls_back_when_path_missing() {
        let channel = detect_install_channel(
            Path::new("/definitely/not/a/real/hekma/bin/hekma"),
            &FakeProbe::default(),
        );
    ...
    fn test_detect_install_channel_wrapper_uses_cargo_home_env() {
        ...
        let exe = cargo_home.join("bin").join("hekma");
        std::fs::create_dir_all(exe.parent().unwrap()).unwrap();
        std::fs::write(&exe, b"hekma").unwrap();
    ...
    fn test_detect_install_channel_uses_cargo_home_and_home_default() {
        let custom = detect_install_channel_with_env(
            Path::new("/custom/cargo/bin/hekma"),
