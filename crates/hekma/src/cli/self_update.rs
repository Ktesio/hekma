...
pub fn run() -> Result<(), Box<dyn std::error::Error>> {
    let exe_path = std::env::current_exe().map_err(|error| SelfUpdateFailed {
        message: format!("Could not locate current hekma executable: {error}"),
    })?;
...
struct ReleaseTarget {
    ...
    /// `hkm` must still refresh `hekma` (and vice versa), because an
    ...
    B: BinaryInstaller,
{
    match channel {
        InstallChannel::Homebrew => {
            ...
            ui::success("Updated Hekma with Homebrew.");
        ...
        InstallChannel::Cargo => {
            runner
                ...
                .map_err(self_update_error)?;
            ui::success("Updated Hekma with Cargo.");
            Ok(SelfUpdateOutcome::Updated(channel))
        }
    ...
    B: BinaryInstaller,
{
    ...
    let asset = format!("hekma-{latest_tag}-{}.{}", target.triple, target.extension);
    let asset_url = format!("{RELEASE_BASE_URL}/{latest_tag}/{asset}");
    let checksum_url = format!("{asset_url}.sha256");

    ui::info(format!(
        "Downloading Hekma {latest_tag} for {}.",
        target.triple
    ));
...
const UNIX_BINARIES: &[&str] = &["hekma", "hkm"];
/// The two shipped binaries on Windows.
const WINDOWS_BINARIES: &[&str] = &["hekma.exe", "hkm.exe"];
...
fn release_target(platform: &Platform) -> Result<ReleaseTarget, SelfUpdateFailed> {
    match (platform.os.as_str(), platform.arch.as_str()) {
        ...
        _ => Err(SelfUpdateFailed {
            message: format!(
                "No prebuilt Hekma binary is available for {}/{}. Install Rust and run: cargo install hekma --force",
...
fn extract_binaries(
    ...
    // `hekma`/`hkm` on different versions.
...
impl ReleaseClient for UreqReleaseClient {
    fn latest_release_tag(&self) -> Result<String, String> {
        let mut response = self
            ...
            .header("User-Agent", concat!("hekma/", env!("CARGO_PKG_VERSION")))
            .call()
...
impl BinaryInstaller for FileBinaryInstaller {
    fn replace_current_binaries(
        ...
        if !current_exe.is_file() {
            return Err(format!(
                "Refusing to replace missing hekma executable at {}.",
                current_exe.display()
            ));
        }
...
mod tests {
    ...
    fn unix_release_payloads(hekma: Vec<u8>, hkm: Vec<u8>) -> Vec<(&'static str, Vec<u8>)> {
        vec![("hekma", hekma), ("hkm", hkm)]
    }

    fn windows_release_payloads(hekma: Vec<u8>, hkm: Vec<u8>) -> Vec<(&'static str, Vec<u8>)> {
        vec![("hekma.exe", hekma), ("hkm.exe", hkm)]
    }
    ...
    fn test_self_update_homebrew_runs_brew_upgrade() {
        ...
        let outcome = run_with_dependencies(
            Path::new("/opt/homebrew/Cellar/hekma/0.3.1/bin/hekma"),
            "0.3.1",
    ...
    fn test_self_update_cargo_runs_cargo_install_force() {
        ...
        let outcome = run_with_channel(
            InstallChannel::Cargo,
            Path::new("/Users/alice/.cargo/bin/hekma"),
            "0.3.1",
            &platform("linux", "x86_64"),
            &runner,
            &release,
    ...
    fn test_self_update_manual_downloads_verifies_and_replaces_binary() {
        let payloads =
            unix_release_payloads(b"new hekma binary".to_vec(), b"new hkm binary".to_vec());
        let archive =
            tar_gz_with_binaries(&[("hekma", b"new hekma binary"), ("hkm", b"new hkm binary")]);
        let checksum = format!("{}  archive.tar.gz\n", sha256_hex(&archive));
        let asset_url =
            format!("{RELEASE_BASE_URL}/v0.4.0/hekma-v0.4.0-x86_64-unknown-linux-gnu.tar.gz");
        let checksum_url = format!("{asset_url}.sha256");
        ...
        // a manual update must never strand `hkm` (or `hekma`) at the
        // old version.
        assert_eq!(
            installer.replacements.borrow().as_slice(),
            &[(PathBuf::from("/usr/local/bin/hekma"), payloads)]
        );
    }
    ...
    fn test_self_update_manual_windows_downloads_zip_asset() {
        let payloads = windows_release_payloads(
            b"windows hekma binary".to_vec(),
            b"windows hkm binary".to_vec(),
        );
        let archive = zip_with_binaries(&[
            ("hekma.exe", b"windows hekma binary"),
            ("hkm.exe", b"windows hkm binary"),
        ]);
        let checksum = format!("{}  archive.zip\n", sha256_hex(&archive));
        let asset_url =
            format!("{RELEASE_BASE_URL}/v0.4.0/hekma-v0.4.0-x86_64-pc-windows-msvc.zip");
        let checksum_url = format!("{asset_url}.sha256");
        let release = FakeReleaseClient::new("v0.4.0")
            .with_download(&asset_url, archive)
            .with_download(&checksum_url, checksum.into_bytes());
        ...
        let outcome = run_with_channel(
            InstallChannel::Manual,
            Path::new("C:/Users/Alice/bin/hekma.exe"),
            "0.3.1",
        ...
        assert_eq!(
            installer.replacements.borrow().as_slice(),
            &[(PathBuf::from("C:/Users/Alice/bin/hekma.exe"), payloads)]
        );
    }
    ...
    fn test_self_update_manual_checksum_mismatch_fails() {
        let archive = tar_gz_with_binary("hekma", b"new hekma binary");
        let asset_url =
            format!("{RELEASE_BASE_URL}/v0.4.0/hekma-v0.4.0-x86_64-unknown-linux-gnu.tar.gz");
        let checksum_url = format!("{asset_url}.sha256");
        let release = FakeReleaseClient::new("v0.4.0")
    ...
    fn test_verify_checksum_rejects_invalid_checksum_file() {
        let error = verify_checksum(b"archive", b"not-a-sha", "hekma.tar.gz").unwrap_err();
    ...
    fn test_self_update_manual_unsupported_target_fails_with_cargo_hint() {
        ...
        assert!(error.message.contains("No prebuilt Hekma binary"));
        assert!(error.message.contains("cargo install hekma --force"));
    }
    ...
    fn test_self_update_manual_newer_prerelease_updates_release() {
        let payloads = unix_release_payloads(
            b"release candidate hekma binary".to_vec(),
            b"release candidate hkm binary".to_vec(),
        );
        let archive = tar_gz_with_binaries(&[
            ("hekma", b"release candidate hekma binary"),
            ("hkm", b"release candidate hkm binary"),
        ]);
        let checksum = format!("{}  archive.tar.gz\n", sha256_hex(&archive));
        let asset_url = format!(
            "{RELEASE_BASE_URL}/v0.4.0-rc.1/hekma-v0.4.0-rc.1-x86_64-unknown-linux-gnu.tar.gz"
        );
        let checksum_url = format!("{asset_url}.sha256");
    ...
    fn test_file_binary_installer_replaces_both_binaries() {
        ...
        let exe = dir.path().join("hekma");
        fs::write(&exe, b"old hekma binary").unwrap();
        // `hkm` may not exist yet (first update after a rename-era
        // install); the installer creates it beside the current exe.
        let hkm = dir.path().join("hkm");

        FileBinaryInstaller
            .replace_current_binaries(
                &exe,
                &[
                    ("hekma", b"new hekma binary".to_vec()),
                    ("hkm", b"new hkm binary".to_vec()),
                ],
            )
            .unwrap();

        assert_eq!(fs::read(&exe).unwrap(), b"new hekma binary");
        assert_eq!(fs::read(&hkm).unwrap(), b"new hkm binary");
        ...
        #[cfg(unix)]
        {
            ...
            assert_eq!(
                fs::metadata(&hkm).unwrap().permissions().mode() & 0o777,
                0o755
            );
        }
    }
    ...
    fn test_file_binary_installer_second_rename_failure_is_reported_per_binary() {
        ...
        // rename (hekma) succeeds and the second (hkm) cannot. The
        ...
        // hekma (renamed before the failure) holds the NEW bytes.
        assert_eq!(fs::read(&exe).unwrap(), b"new hekma binary");
    }
    ...
    fn test_file_binary_installer_rejects_missing_exe() {
        let dir = tempfile::TempDir::new().unwrap();
        let exe = dir.path().join("missing-hekma");

        let error = FileBinaryInstaller
            .replace_current_binaries(&exe, &[("hekma", b"new hekma binary".to_vec())])
            .unwrap_err();

        assert!(error.contains("Refusing to replace missing hekma executable"));
    }
        ...
        for os_arch in [
            ...
            assert_eq!(
                release_target(&platform(os_arch.0, os_arch.1))
                    ...
                    .binary_names,
                &["hekma", "hkm"],
                "unix target {os_arch:?} must ship hekma + hkm"
            );
        }
        assert_eq!(
            release_target(&platform("windows", "x86_64"))
    ...
    fn test_extract_binary_reports_missing_binary() {
        let archive = tar_gz_with_binary("not-hekma", b"nope");
        let error = extract_binaries(
            &archive,
            &ReleaseTarget {
                triple: "x86_64-unknown-linux-gnu",
                extension: "tar.gz",
                binary_names: &["hekma", "hkm"],
            },
        )
        .unwrap_err();

        assert!(error.message.contains("did not contain hekma"));
        assert!(error.message.contains("hkm"));
    }

    #[test]
    fn test_extract_zip_reports_missing_binary() {
        let archive = zip_with_binary("not-hekma.exe", b"nope");
        let error = extract_binaries(
            &archive,
            &ReleaseTarget {
                triple: "x86_64-pc-windows-msvc",
                extension: "zip",
                binary_names: &["hekma.exe", "hkm.exe"],
            },
        )
        .unwrap_err();

        assert!(error.message.contains("did not contain hekma.exe"));
        assert!(error.message.contains("hkm.exe"));
    }
    ...
    fn test_truncated_tar_gz_download_fails_instead_of_yielding_a_partial_binary() {
        ...
        // so the entry for `hekma` is found and extraction begins — the failure only
        ...
        // user's `hekma`. (The checksum gate would also catch this, but only because
        ...
        let full = tar_gz_with_binaries(&[("hekma", &payload), ("hkm", &payload)]);
        let truncated = full[..full.len() / 4].to_vec();

        let error = extract_binaries(&truncated, &target).unwrap_err();

        assert!(
            error.message.contains("Could not extract hekma"),
            "{}",
            error.message
        );
        ...
        assert!(extracted
            .iter()
            .any(|(name, bytes)| *name == "hekma" && bytes == &payload));
        assert!(extracted
            .iter()
            .any(|(name, bytes)| *name == "hkm" && bytes == &payload));
    }
    ...
    fn test_corrupted_zip_entry_fails_the_integrity_check_on_extract() {
        ...
        // must fail rather than return silently-wrong bytes for `hekma.exe`.
        let target = ReleaseTarget {
            triple: "x86_64-pc-windows-msvc",
            extension: "zip",
            binary_names: &["hekma.exe", "hkm.exe"],
        ...
        let mut archive = zip_with_binaries(&[("hekma.exe", &payload), ("hkm.exe", &payload)]);
        ...
        let intact = extract_binaries(
            &zip_with_binaries(&[("hekma.exe", &payload), ("hkm.exe", &payload)]),
            &target,
        )
        .unwrap();
    ...
    fn test_a_failed_checksum_never_reaches_the_installer() {
        ...
        let archive = tar_gz_with_binary("hekma", b"tampered hekma binary");
        let asset_url =
            format!("{RELEASE_BASE_URL}/v0.4.0/hekma-v0.4.0-x86_64-unknown-linux-gnu.tar.gz");
        let checksum_url = format!("{asset_url}.sha256");
        let release = FakeReleaseClient::new("v0.4.0")
    ...
    fn test_checksum_accepts_an_uppercase_digest_from_the_sha256_file() {
        ...
        let uppercase = format!("{}  hekma.tar.gz\n", digest.to_ascii_uppercase());

        verify_checksum(archive, uppercase.as_bytes(), "hekma.tar.gz").unwrap();
        ...
        let wrong = format!("{}  hekma.tar.gz\n", "A".repeat(64));
        let error = verify_checksum(archive, wrong.as_bytes(), "hekma.tar.gz").unwrap_err();
        assert!(error.message.contains("Checksum verification failed"));
    }
}
