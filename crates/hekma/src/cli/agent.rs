//! `hekma agent register | remove | list` — thin CLI over the engine's
...
//! into `miette` diagnostics with remediation hints (miette lives in `hekma`
...
pub fn register(name: &str, adapter: &AdapterArg) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match engine.register_with_adapter(name, &adapter.to_ref()) {
        Ok(instance) => {
            ...
            // what `hekma agent show` will render.
...
/// The `hekma agent show <name> --json` document (story 1-7, AD-14).
///
/// A versioned wrapper carrying the SAME [`FLEET_SCHEMA_VERSION`] as the
/// `list --json` [`FleetListing`] (so `hekma --json` speaks ONE schema, AD-14) plus
...
/// The `hekma agent usage <name> --json` document (story 4-3, FR-22/FR-26, AD-14).
///
...
/// The Fleet-wide `hekma agent usage --json` document (no name — story 4-3, FR-22).
///
...
/// The `hekma agent memory attach --json` document (story 6-6, AD-14).
///
...
/// The `hekma agent memory detach --json` document (story 6-6, AD-14). The
...
/// carries NO path — `hekma` never constructs the managed-directory name itself
...
///
/// Most `hekma` commands (`logs`, `stop`, `pause`, the human `show`, …) pass the raw
...
/// the engine's PUBLIC [`hekma_engine::InstanceName`] newtype, the SAME rule the
/// engine applies internally, so `hekma` re-derives nothing (AD-2) — makes the code
/// uniformly `2` for every command.
fn validate_instance_name(name: &str) -> Result<(), Box<dyn std::error::Error>> {
    hekma_engine::InstanceName::new(name)
        .map(|_| ())
        .map_err(|reason| {
...
/// `hekma agent show <name> [--json]` — render an instance's effective Capability
...
fn render_runtime_status(status: &hekma_engine::InstanceStatus, entry: Option<&FleetEntry>) {
    ...
    if status.instance.state == hekma_engine::LifecycleState::Failed {
...
const EMPTY_FLEET_HINT: &str =
    "No Agent Instances registered yet. Register one with: hekma agent register <name> --kind <kind>";

/// The `hekma agent list` Budget column HEADER (story 3-3, FR-23/AD-8).
///
...
/// The `hekma agent list` Usage column HEADER (story 11-4, AI-45 — the Usage-side
...
/// — e.g. `$0.20/$0.50 (estimated)`. The SOLE currency formatting in `hekma` routes
...
/// Render the Fleet-WIDE total footer for the human `hekma agent list` (story 3-5,
...
///   `domain` aggregate (`hekma` only renders it, with the singular "1 instance unpriced").
...
/// `hekma agent list [--json]` — render the Fleet (FR-4).
///
...
/// [`FleetListing`] document to STDOUT and nothing else there (AD-14: `hekma --json`
...
/// persisted state via [`hekma_engine::Engine::fleet`] — there is no cache, so
...
pub fn list(json: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // ledger query, `hekma` never sums the ledger itself (AD-2).
    ...
    // appears): the active Metering Source is surfaced on `hekma agent show` (the
    // detail row), `hekma agent list --json` (the `metering_source` field), and
    // `hekma agent usage` (named + JSON forms). It is NOT a human-`list` column: the
...
/// `hekma agent usage [<name>] [--json]` — a focused, scriptable read of the Usage
...
/// `hekma agent start <name> [--detach]` — start a registered Agent Instance
/// (AC1/AC2).
///
...
/// handle drop, a standalone `hekma agent start <name>` stops the agent when this
...
/// spawn, so it survives this command's exit and the next `hekma` command
            ...
            if detach {
                ...
                ui::note(
                    "detached: the agent keeps running after this command exits, and the \
                     next `hekma` command re-adopts it. Until then it is NOT supervised: no \
...
/// `hekma agent stop <name> [--timeout <secs>]` — stop a running Agent Instance
/// (AC3/AC4).
///
...
/// `hekma agent pause <name>` — pause a running Agent Instance with honest, per-OS
...
/// `hekma agent resume <name>` — resume a paused Agent Instance (AC2/AC6).
///
...
/// `hekma agent send <name> <text>` — send text input to a running Agent
...
/// How often `hekma agent logs --follow` polls for new output (story 4-2,
...
/// two separate steps within the SAME call, not one atomic unit. `hekma agent
/// logs --follow` runs in a SEPARATE process from whatever transitions the
/// instance (e.g. a concurrent `hekma agent stop`), so it can observe the NEW
...
/// `crates/hekma-engine/tests/logs.rs`'s
...
/// `hekma agent logs <name> [--follow]` — read retained logs, optionally
/// following live output (story 4-2, FR-25, spine AD-12).
///
/// One-shot: dumps every currently-retained [`LogLine`] to stdout (this IS
/// the command's result — AD-12's "stdout of hekma is command output"
...
/// its own `schema_version` ([`hekma_engine::LOG_SCHEMA_VERSION`]), so per-line
...
///
/// A downstream consumer that STOPS READING (`hekma agent logs --json | head -5`)
...
/// (fix pass, M1).
///
/// A closed downstream pipe (`hekma agent logs --json | head -5`) is NOT an error: the
...
/// write error and Rust ignores `SIGPIPE` — so `hekma agent logs --json | head -5`
...
/// [`LogLine`] (its `schema_version` is [`hekma_engine::LOG_SCHEMA_VERSION`]) — no
...
fn note_if_rotated(next_cursor: u64, prior_cursor: u64, name: &str) {
    if next_cursor < prior_cursor {
        ui::note(format!(
            "output rotated; a small window may be missing from the live tail — `hekma agent logs {name}` \
...
/// extra read via the same `effective_capabilities` mechanism `hekma agent show`
...
fn note_if_best_effort(facade: &hekma_engine::Blocking<'_>, name: &str, op: &str) {
...
/// `hekma agent config set <name> <key> <value>` — write one key to the Agent
...
/// `hekma agent config get <name> [<key>] [--json]` — read the EFFECTIVE (resolved)
...
/// `invocation-override`), read from the [`hekma_engine::SourceLayer`] tag the
/// engine records per leaf (AD-2: `hekma` never re-derives it). Deep-resolved via the
...
/// engine's [`hekma_engine::ResolvedValue::display`] masks them — `hekma` renders
...
pub fn config_get(
    ...
    // (hekma never resolves secrets itself, AD-2). A live-resolution failure surfaces
                ...
                return Err(AgentUnknownConfigKey {
                    message: format!(
                        "Agent Instance '{name}' has no effective value for config key '{key}'. \
                         List the effective config with: hekma agent config get {name}"
                    ),
                }
                .into());
            }
...
fn table_prefix_diagnostic(name: &str, key: &str, effective: &EffectiveConfig) -> Option<String> {
    ...
    Some(format!(
        "Agent Instance '{name}' has no effective value for config key '{key}' — it is a \
         ...
         e.g.: hekma agent config get {name} {}",
        named.join(", "),
        named[0]
    ))
}
...
/// The `hekma agent config get --json` document (story 2-3, AC4 / AD-12).
///
...
/// [`hekma_engine::SourceLayer`] wire label; `unvalidated` is the story-2-2
...
/// leaf's [`hekma_engine::ResolvedValue::display`] — and every source via the
/// winning layer tag's `as_str` ([`hekma_engine::SourceLayer::as_str`]) — `hekma`
...
fn config_json(
    ...
    let entries: Vec<ConfigLeaf> = effective
        ...
        .map(|(key, resolved)| ConfigLeaf {
            ...
            // MASKS a secret by default, so `hekma` needs no `toml` dep and cannot leak
...
/// engine's [`EffectiveConfig::is_unvalidated`] accessor (so `hekma` owns no config
...
/// marked validated. The "Source" column shows the winning [`hekma_engine::SourceLayer`]
/// label (`engine-default` / `kind-default` / `instance` / `invocation-override`),
/// read per leaf from the engine's `source` tag (AD-2: `hekma` never re-derives it).
...
fn render_effective_config(
    ...
    let rows: Vec<Vec<ui::TableCell>> = effective
        ...
        .map(|(key, resolved)| {
            ...
            // engine accessor (AD-2: `hekma` never re-implements the boundary). A
            ...
            // The engine renders the value (no `toml::Value` in `hekma` — AD-2),
            ...
            vec![
                ...
                // `hekma` never re-derives it (AD-2).
...
/// `hekma agent memory attach <name> --kind <kind> [--json]` — attach a Memory
...
/// command only displays the path the ENGINE returned (DC-1 — `hekma` never joins
...
pub fn memory_attach(name: &str, kind: &str, json: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if json {
        ...
        // engine (hekma re-derives neither — AD-2). JSON-only: the human path
                ...
                return Err(AgentIo {
                    message: format!(
                        "Attached the Memory Backing to '{name}', but the engine reports no \
                         attachment; re-run hekma agent memory attach {name} --kind {kind} --json."
                    ),
                }
                .into())
            }
    ...
    AgentIo {
        message: format!(
            "Attached the Memory Backing to '{name}', but reading the attachment back failed: \
             ...
             `hekma agent memory attach {name} --kind {kind} --json` (a same-kind re-attach is an \
             idempotent success) or undo it with `hekma agent memory detach {name}`."
        ),
    }
}

/// `hekma agent memory detach <name> [--json]` — detach an Agent Instance's Memory
...
fn map_config_error(err: ConfigError) -> Box<dyn std::error::Error> {
    match err {
        ...
        ConfigError::UnknownKey { .. } => AgentUnknownConfigKey {
            message: format!(
                "{err}. Set a known unified key, or use the agent.* pass-through namespace for \
                 agent-native extras (e.g. hekma agent config set <name> agent.<key> <value>)."
            ),
        }
        .into(),
        ...
        ConfigError::NotFound { name } => AgentNotFound {
            message: format!(
                "No Agent Instance named '{name}' is registered. List the Fleet with: hekma agent list"
            ),
        }
        .into(),
...
/// engine owns its tokio runtime; `hekma` drives it through the blocking facade.
...
fn map_error(err: RegistryError) -> Box<dyn std::error::Error> {
    match err {
        RegistryError::DuplicateName { name } => AgentDuplicateName {
            message: format!(
                "An Agent Instance named '{name}' already exists. Choose a different name, \
                 or remove the existing instance with: hekma agent remove {name}"
            ),
        }
        .into(),
        ...
        RegistryError::RunningRequiresForce { name } => AgentRunningRequiresForce {
            message: format!(
                "Agent Instance '{name}' is running. Stop it first, or pass --force to remove \
                 it anyway: hekma agent remove {name} --delete --force"
            ),
        }
        .into(),
        ...
        RegistryError::RegisterOrphanRow {
            ...
            message: format!(
                "Agent Instance '{name}' left an orphaned registry row: its Agent Home could not \
                 ...
                 ({rollback_error}). Remove the stale entry with: hekma agent remove {name} --force"
            ),
        }
        .into(),
        ...
        // in hekma-adapter-api), so this arm adds only the remediation.
        ...
        RegistryError::NoMeteringSource { adapter } => AgentNoMeteringSource {
            message: format!(
                "Adapter '{adapter}' declares no viable Metering Source. Add a `[metering]` \
                 section with source = \"self-reported\" or \"engine-observed\" — Hekma rejects \
        ...
        RegistryError::MemoryBackingHotSwap { name, state } => AgentMemoryHotSwap {
            message: format!(
                "Agent Instance '{name}' is '{state}'; a Memory Backing cannot be hot-swapped. \
                 ...
                 to a terminal state first: hekma agent stop {name} from running or paused"
            ),
        }
        .into(),
        RegistryError::MemoryBackingKindConflict {
            ...
            message: format!(
                "Agent Instance '{name}' already has a '{attached}' Memory Backing attached; \
                 detach it before attaching '{requested}': hekma agent memory detach {name}"
            ),
        }
        .into(),
...
fn map_engine_error(err: EngineError) -> Box<dyn std::error::Error> {
    match err {
        ...
        EngineError::InvalidTransition(inner) => AgentInvalidTransition {
            message: format!("{inner}. Check the instance's current state with: hekma agent list"),
        }
        .into(),
        ...
        // (the level + OS) and pointing at `hekma agent show`. No state changed.
        EngineError::CapabilityUnsupported {
            ...
            message: format!(
                "Agent Instance '{name}' cannot {capability}: this agent declares {capability} \
                 '{level}' on {os}. Inspect its Capability Declaration with: hekma agent show {name}"
            ),
        }
        .into(),
        ...
        EngineError::ResumeUnsupported { name, os, level } => AgentResumeUnsupported {
            message: format!(
                "Agent Instance '{name}' is paused, but this agent declares pause '{level}' \
                 ...
                 hekma agent stop {name} && hekma agent start {name}. The same resume does work on \
        ...
        // is `hekma agent resume` — suggesting `hekma agent start` there hits a
        ...
        EngineError::NotRunning { name, state } => {
            let remediation = if state == "paused" {
                format!("resume it with: hekma agent resume {name}")
            } else {
                format!("start it first with: hekma agent start {name}")
            };
            AgentNotRunning {
                message: format!(
                    "Agent Instance '{name}' is not running (current state: {state}); \
                     {remediation}. List the Fleet with: hekma agent list"
                ),
            }
            .into()
        }
        ...
        EngineError::InteractionUnavailable { name, detail } => AgentInteractionUnavailable {
            message: format!(
                "Agent Instance '{name}' cannot receive input right now: {detail}. Durable \
                 ...
                 future release); within a single `hekma` process/embedding session this works."
            ),
        }
        .into(),
        ...
        EngineError::InteractionTimedOut { name, timeout_secs } => AgentInteractionTimedOut {
            message: format!(
                "Agent Instance '{name}' is not draining its input within {timeout_secs}s and \
                 ...
                 session; restart it for a fresh one: hekma agent stop {name} && hekma agent start \
                 {name}"
            ),
        }
        .into(),
        ...
        EngineError::StopUnconfirmed { name, timeout_secs } => AgentStopUnconfirmed {
            message: format!(
                "Agent Instance '{name}' was sent SIGKILL but has not been confirmed dead \
                 ...
                 hekma agent stop {name} — this will not re-block if it is still stuck, and will \
...
mod tests {
    ...
    fn map_error_includes_remediation_hints() {
        ...
        assert!(dup.to_string().contains("hekma agent remove demo"));
        ...
        let invalid = map_error(RegistryError::InvalidName {
            name: "Bad".into(),
            reason: hekma_engine::NameError::BadChar,
        });
        ...
        assert!(missing.to_string().contains("hekma agent list"));
        ...
        assert!(orphan_msg.contains("hekma agent remove demo --force"));

        // Store errors surface as a state-store diagnostic.
        let store = map_error(RegistryError::Store(
            hekma_engine::ports::StoreError::Backend("db gone".into()),
        ));
    ...
    fn map_error_renders_the_contract_incompatible_rejection() {
        ...
        // `incompatible_major_fails_naming_both_versions_and_the_rule` (hekma
    ...
    fn attach_readback_failure_names_the_persisted_attachment() {
        ...
        assert!(
            msg.contains("hekma agent memory attach demo --kind filesystem --json"),
            "gives the re-run remediation: {msg}"
        );
        assert!(
            msg.contains("hekma agent memory detach demo"),
    ...
    fn map_engine_error_renders_the_interaction_timed_out_diagnostic() {
        ...
        // spawned it, but a single `hekma agent send` invocation opens its OWN
        // fresh `Engine` and exits after one call; a SEPARATE `hekma agent
        ...
        assert!(
            msg.contains("hekma agent stop stuck") && msg.contains("hekma agent start stuck"),
        ...
        let cases: Vec<(&str, EngineError, ExitCode)> = vec![
            ...
            (
                ...
                EngineError::InvalidName {
                    ...
                    reason: hekma_engine::NameError::BadFirstChar,
                },
                ExitCode::Usage,
            ),
            (
    ...
    fn map_engine_error_renders_the_stop_unconfirmed_diagnostic() {
        ...
        assert!(
            msg.contains("hekma agent stop stuck"),
    ...
    fn sample_fleet_entry(name: &str) -> FleetEntry {
        FleetEntry {
            name: hekma_engine::InstanceName::new(name).unwrap(),
            kind: "mock".to_string(),
            state: hekma_engine::LifecycleState::Registered,
            restart_count: 0,
            restart_policy: hekma_engine::RestartPolicy::OnFailure,
            failed_cause: None,
            budget: None,
            usage: UsageView::new(
    ...
    fn sample_effective() -> EffectiveConfig {
        use hekma_engine::{resolve, SourceLayer};
        let layers = [
            ConfigLayer::parse(
    ...
    fn table_prefix_diagnostic_names_the_child_leaves_sorted_and_capped() {
        ...
        assert!(
            message.contains("hekma agent config get demo budget.breach_action"),
    ...
    fn config_get_reveal_overlays_resolved_cleartext_in_process() {
        ...
        }
        {
            ...
            let revealed = blocking
                .reveal_secrets("sec", hekma_engine::ConfigLayer::empty())
                .unwrap();
        ...
        let eff = engine
            .blocking()
            .effective_config("sec", hekma_engine::ConfigLayer::empty())
            .unwrap();
    ...
    fn render_capabilities_handles_empty_and_nonempty() {
        use hekma_engine::{Capability, OsId, SupportLevel};
    ...
    fn budget_view(
        ...
        breach_action: hekma_engine::BreachAction,
    ) -> BudgetView {
        BudgetView {
    ...
    fn budget_cell_renders_the_per_run_scope_before_the_cumulative_scope() {
        ...
        let budget = budget_view(
            ...
            None,
            None,
            hekma_engine::BreachAction::Pause,
        );
    ...
    fn budget_cell_keeps_the_estimate_label_out_of_the_truncatable_list_cell() {
        ...
        let budget = budget_view(
            ...
            hekma_engine::BreachAction::Stop,
        );
    ...
    fn budget_cell_with_no_configured_scope_still_names_the_breach_action() {
        ...
        let actionless = budget_view(
            ...
            None,
            None,
            hekma_engine::BreachAction::Warn,
        );
    ...
    fn a_broken_stdout_pipe_stops_cleanly_while_a_real_io_failure_still_diagnoses() {
        // Fix-pass M1 contract: `hekma agent logs --json | head -5` must exit 0. Rust
    ...
    fn a_failed_secret_reveal_diagnoses_without_leaking_the_secret_value() {
        ...
        // resolvers tried but no value; `hekma` appends the remediation. This test
    ...
    fn the_remaining_engine_error_arms_map_to_their_documented_diagnostic_types() {
        ...
        let backend = map_engine_error(EngineError::Backend {
            name: "demo".to_string(),
            source: hekma_engine::ports::BackendError::Spawn {
        ...
        let store = map_engine_error(EngineError::Store(
            hekma_engine::ports::StoreError::CorruptRow {
                name: "demo".to_string(),
