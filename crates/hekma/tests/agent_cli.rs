//! Integration tests for `hekma agent register | remove | list`.
//!
//! These drive the real `hekma` binary (via `CARGO_BIN_EXE_hekma`) with
...
use hekma_conformance::test_support::{current_os_key, ManifestFixture};
use hekma_conformance::uj3;
use hekma_engine::{Engine, FleetEntry, LifecycleState, UsageView};
use helpers::{run_hekma_agent, run_hekma_agent_with_env, KtRun, TestContext};
...
/// the next `hekma` invocation (story 1-6). A normal `hekma agent start` cleanly drops
...
fn register_prints_home_path_and_exits_zero() {
    ...
    // every other `hekma` command's `ui::success` usage. AD-12 reserves stderr for
...
fn duplicate_registration_exits_nonzero_with_diagnostic() {
    ...
    assert!(second.stderr.contains("hekma agent remove demo"));
}
...
fn register_hermes_kind_plumbs_through_the_cli_and_surfaces_in_list() {
    ...
    let listed = run_hekma_agent(&["agent", "list", "--json"], &ctx.project_dir, state_dir);
    assert!(
        listed.success,
...
fn list_shows_registered_instances() {
    ...
    // Empty Fleet first.
    let empty = run_hekma_agent(&["agent", "list"], &ctx.project_dir, state_dir);
    ...
    let listed = run_hekma_agent(&["agent", "list"], &ctx.project_dir, state_dir);
...
fn remove_running_without_force_exits_nonzero_and_with_force_succeeds() {
    ...
    let still = run_hekma_agent(&["agent", "list"], &ctx.project_dir, state_dir);
    assert!(still.stdout.contains("live"));

    // With --force: succeeds.
    let forced = run_hekma_agent(
...
// ---- Story 1.3: manifest adapters + `hekma agent show` ----
...
/// `hekma_conformance::test_support::ManifestFixture` (story 10-1): this
...
fn register_requires_kind_or_manifest() {
    ...
    let run = run_hekma_agent(&["agent", "register", "x"], &ctx.project_dir, state_dir);
...
fn show_renders_effective_capabilities() {
    ...
    let show = run_hekma_agent(&["agent", "show", "demo"], &ctx.project_dir, state_dir);
...
fn show_unknown_instance_exits_nonzero() {
    ...
    let show = run_hekma_agent(&["agent", "show", "ghost"], &ctx.project_dir, state_dir);
...
// ---- Story 1.4: `hekma agent start` / `hekma agent stop` ----
...
/// `hekma_conformance::test_support` states the manifest shape ONCE); this
/// wrapper only keeps the fixture's `fake-agent-adapter` subdirectory name.
/// The locator behind it is the SHARED `hekma_conformance::fake_agent_bin`:
/// the running hekma integration-test binary resolves its own
/// `target/<profile>/deps/` sibling — the same profile dir the old local
/// `CARGO_BIN_EXE_hekma`-anchored copy named — so that copy was duplication,
/// not a distinct seam (the `CARGO_BIN_EXE_hekma` seam for RUNNING hekma is
/// separate and stays).
...
fn start_prints_running_state_and_exits_zero() {
    ...
    //
    // NOTE (single-lifetime boundary): each `hekma` invocation is its own engine
    // lifetime; the started process is cleaned up on exit (kill-on-drop). A
    // separate `hekma agent stop` cannot re-attach to it (orphan adoption is story
    ...
    let run = run_hekma_agent(&["agent", "start", "svc"], &ctx.project_dir, state_dir);
...
fn start_prints_single_lifetime_notice_to_stderr_only() {
    // LOW-1, story-12-1 flip: the success path of a PLAIN `hekma agent start` is
    // honest about single-lifetime supervision — a standalone `hekma agent start`
...
/// Run `hekma` with a HARD WALL-CLOCK BOUND and a forensic dump on timeout
/// (the 12-1 detach test's Windows hang triage): on timeout the child is
/// killed and the harness reports whether hekma was STILL ALIVE, its partial
...
/// of the file keeps the plain [`helpers::run_hekma_agent`].
...
fn run_hekma_agent_bounded(
    ...
    // the pipe, or a half-dead hekma, can hold it open indefinitely).
    ...
    let status = loop {
        if let Some(status) = child.try_wait().expect("try_wait hekma") {
            break Some(status);
        }
    ...
    // On timeout, KILL hekma, give the pipes a 1s grace to EOF, then read the
    ...
    if timed_out {
        ...
        let hekma_gone = child.try_wait().map(|s| s.is_some()).unwrap_or(false);
        println!(
            "KT HUNG after {:?}: args={:?} hekma_alive_at_kill=true hekma_dead_after_kill={} \n\
             ---- hekma partial stdout ----\n{}\n\
             ---- hekma partial stderr ----\n{}",
            bound,
            args,
            hekma_gone,
    ...
    // hekma has exited (or been killed) — give the readers a short grace to
    // see EOF, then PROCEED WITHOUT THEM. On windows-latest this harness
    // proved the pipes never EOF after a detached start even though hekma is
    // DEAD: the surviving detached agent ends up holding hekma's stdio pipe
    ...
    // a script that captures `hekma agent start --detach` output would hang
    // until the agent exits.) The leaked readers die with the test process;
    // the incremental buffers already hold hekma's full transcript.
...
fn start_detach_survives_the_command_exit_and_the_next_command_stops_it() {
    // Story 12-1, the acceptance criterion AT THE CLI, cross-OS: `hekma agent
    ...
    // a BENIGN intervening command (`hekma agent list` — the 12-1 AMENDMENT leg)
    // adopts the child DISARMED and leaves it alive at its exit; and a later
    // `hekma agent stop` re-adopts the live process and lands the terminal state
    ...
    let reg = run_hekma_agent_bounded(
        &[
            "agent",
            "register",
            "detachy",
    ...
    let run = run_hekma_agent_bounded(
    ...
    assert!(
        !run.timed_out,
        "THE START LEG HUNG (hekma alive at kill={}): hekma stdout={}\nkt stderr={}\n\
         if `running` printed, the ENGINE finished and hekma's exit path blocks;\n\
    ...
    assert!(
        pid_alive(pid),
        "the detached child must be alive after `hekma agent start --detach` exited"
    );
    // 12-1 AMENDMENT (review loop 1): a BENIGN intervening command must not
    // kill the detached agent. `hekma agent list` opens the engine, adopt_orphans
    ...
    let list = run_hekma_agent_bounded(&["agent", "list"], &ctx.project_dir, state_dir, LEG_BOUND);
    println!("detach-leg: list returned success={}", list.run.success);
    assert!(
        !list.timed_out,
        "THE LIST LEG HUNG (hekma alive at kill={}): stdout={} stderr={}",
        list.alive_at_kill, list.run.stdout, list.run.stderr
    );
    assert!(
        list.run.success,
        "the benign `hekma agent list` should succeed; stderr={}",
        list.run.stderr
    );
    assert!(
    ...
    assert!(
        pid_alive(pid),
        "the benign `hekma agent list` exit must NOT kill the detached agent \
    ...
    let stop = run_hekma_agent_bounded(
        &["agent", "stop", "detachy"],
        &ctx.project_dir,
        state_dir,
        LEG_BOUND,
    ...
    assert!(
        !stop.timed_out,
        "THE STOP LEG HUNG (hekma alive at kill={}): stdout={} stderr={}",
...
"#,
    ...
    let show = run_hekma_agent(&["agent", "show", "obsdet"], &ctx.project_dir, state_dir);
...
fn start_help_carries_the_enforcement_window_honesty() {
    ...
    // BEFORE running it. `hekma agent start --help` renders the flag's help text,
...
kind = "bad"
[lifecycle.start]
exec = "hekma-no-such-binary-cli-1-4"
[capabilities.interaction]
linux = "guaranteed"
macos = "guaranteed"
windows = "guaranteed"
...
"#,
    ...
    let run = run_hekma_agent(&["agent", "start", "bad"], &ctx.project_dir, state_dir);
    ...
    assert!(
        run.stderr.contains("hekma-no-such-binary-cli-1-4"),
...
fn stop_on_stopped_returns_uniform_invalid_transition() {
    ...
    let run = run_hekma_agent(&["agent", "stop", "nat"], &ctx.project_dir, state_dir);
...
fn start_unknown_instance_exits_nonzero() {
    ...
    let state = TestContext::new();
    let state_dir = state.project_dir.as_path();
    let run = run_hekma_agent(&["agent", "start", "ghost"], &ctx.project_dir, state_dir);
...
// ---- Story 1.5: `hekma agent pause` / `hekma agent resume` (AC6) ----
...
fn pause_prints_paused_state_and_exits_zero_guaranteed_unix() {
    // AC6 + AC1 at the CLI (Unix guaranteed): `hekma agent pause` on a genuinely
    ...
    // `pause_after_windows_engine_death_reconciles_and_fails_honestly_windows`.
    //
    // NOTE (single-lifetime CLI boundary, story 1-6): each `hekma` command is a
    ...
    // does NOT chain a follow-up `hekma agent resume` (the paused process does not
    ...
    // `hekma agent start` would kill it on the command's clean engine drop.
    start_via_surviving_engine(state_dir, "svc");

    let paused = run_hekma_agent(&["agent", "pause", "svc"], &ctx.project_dir, state_dir);
    assert!(
        paused.success,
    ...
    run_hekma_agent(&["agent", "stop", "svc"], &ctx.project_dir, state_dir);
}
...
fn pause_after_windows_engine_death_reconciles_and_fails_honestly_windows() {
    ...
    // scenario is the one that follows an engine death: the `hekma agent pause`
    ...
    let paused = run_hekma_agent(&["agent", "pause", "be"], &ctx.project_dir, state_dir);
    assert!(
        !paused.success,
...
fn pause_best_effort_prints_qualifier_note_to_stderr_only_unix() {
    ...
    // and survives, then a separate `hekma` command adopts it live. That survival
    ...
    // are fully covered on Windows by `crates/hekma-engine/tests/pause.rs`, and
    ...
    // when pause adopts it (story 1-6); a plain `hekma agent start` kills it on exit.
    start_via_surviving_engine(state_dir, "be");
    ...
    // Teardown.
    run_hekma_agent(&["agent", "stop", "be"], &ctx.project_dir, state_dir);
}
...
fn pause_unsupported_exits_nonzero_quoting_the_declaration_unix() {
    ...
    // unsupported projection) are covered by `crates/hekma-engine/tests/pause.rs`,
    ...
    // declaration (names pause, the OS, the level) and points at `hekma agent show`.
    let ctx = TestContext::new();
    let state = TestContext::new();
    let state_dir = state.project_dir.as_path();
    ...
    let paused = run_hekma_agent(&["agent", "pause", "un"], &ctx.project_dir, state_dir);
    assert!(
        !paused.success,
    ...
    assert!(
        paused.stderr.contains("hekma agent show un"),
        "stderr must point at hekma agent show; stderr={}",
        paused.stderr
    );
    ...
    // follow-up `hekma` command here: each command's clean engine drop kills the
    ...
    // Teardown.
    run_hekma_agent(&["agent", "stop", "un"], &ctx.project_dir, state_dir);
}
...
fn pause_on_registered_returns_uniform_invalid_transition() {
    ...
    let run = run_hekma_agent(&["agent", "pause", "nat"], &ctx.project_dir, state_dir);
...
// ---- Story 4-1: `hekma agent send <name> <text>` (AC-A, AC-B, AC-C) ----
...
fn send_on_an_adopted_instance_exits_nonzero_with_interaction_unavailable_unix() {
    ...
    // dropping its engine, so the SEPARATE `hekma agent send` invocation this
    ...
    // `hekma` binary, which no other test exercises) — never
    ...
    // `crates/hekma-engine/tests/interaction.rs`, which does not cross a
    // process boundary.
    //
...
fn send_unsupported_exits_nonzero_quoting_the_declaration_unix() {
    ...
    assert!(
        sent.stderr.contains("hekma agent show un"),
        "stderr must point at hekma agent show; stderr={}",
        sent.stderr
    );
...
fn send_text_that_looks_like_help_flag_is_sent_literally_not_intercepted_as_cli_help() {
    ...
    // The hyphen-value parse problem (`hekma agent send x "-5 degrees"` used to
...
fn send_on_a_paused_instance_is_not_running_with_resume_remediation_not_start() {
    ...
    // first with: hekma agent start <name>" even for a `paused` instance —
    // but `start` on a paused instance hits `InvalidTransition` (a SECOND,
    // confusing error), since the correct remediation is `hekma agent resume`.
    let ctx = TestContext::new();
    let state = TestContext::new();
    let state_dir = state.project_dir.as_path();
    run_hekma_agent(
    ...
    assert!(
        sent.stderr.contains("hekma agent resume pz"),
        "the remediation must point at resume for a paused instance; stderr={}",
        sent.stderr
    );
    assert!(
        !sent.stderr.contains("hekma agent start pz"),
...
fn list_surfaces_the_restart_count_column() {
    // AC9: `hekma agent list` surfaces the per-instance restart count.
    let ctx = TestContext::new();
    let state = TestContext::new();
    let state_dir = state.project_dir.as_path();
    run_hekma_agent(
...
fn show_surfaces_restart_count_policy_and_failed_cause() {
    // AC9: `hekma agent show` on a failed instance surfaces the restart count, the
    ...
    let show = run_hekma_agent(&["agent", "show", "svc"], &ctx.project_dir, state_dir);
    assert!(show.success, "show should exit 0; stderr={}", show.stderr);
...
fn show_surfaces_a_launch_error_failed_cause() {
    ...
    // written), yet `hekma agent show` must still surface the failed cause — via the
...
[lifecycle.start]
exec = "hekma-no-such-binary-cli-med3"
...
"#;
    ...
    let start = run_hekma_agent(&["agent", "start", "bad"], &ctx.project_dir, state_dir);
    assert!(!start.success, "start of a bad exec should exit non-zero");

    let show = run_hekma_agent(&["agent", "show", "bad"], &ctx.project_dir, state_dir);
    assert!(show.success, "show should exit 0; stderr={}", show.stderr);
    ...
    assert!(
        show.stdout.contains("hekma-no-such-binary-cli-med3"),
...
fn start_restarts_a_failed_instance() {
    // AC3 at the CLI: `hekma agent start` restarts a `failed` instance (the 1-6
...
"#,
    ...
    // `hekma agent start` on a failed instance restarts it → running (exit 0).
    let start = run_hekma_agent(&["agent", "start", "svc"], &ctx.project_dir, state_dir);
    assert!(
        start.success,
...
fn list_json_emits_a_parseable_document_with_budget_seed_and_real_usage() {
    // Story 1-7 (AC5/AC9) + story 3-1 (AC-C/AC11): `hekma agent list --json` writes ONE
    ...
    let run = run_hekma_agent(&["agent", "list", "--json"], &ctx.project_dir, state_dir);
    assert!(
        run.success,
        "list --json should exit 0; stderr={}",
        run.stderr
...
fn human_list_shows_the_budget_column_and_real_usage_columns() {
    ...
    let run = run_hekma_agent(&["agent", "list"], &ctx.project_dir, state_dir);
    ...
    let wide = run_hekma_agent_with_env(
        &["agent", "list"],
        &ctx.project_dir,
        state_dir,
        &[("COLUMNS", "140")],
...
fn show_json_surfaces_the_same_entry_shape_with_budget_seed_and_real_usage() {
    // Story 1-7 (AC5) + story 3-1: `hekma agent show <name> --json` writes ONE JSON
...
fn human_show_surfaces_the_budget_row_and_real_usage_rows() {
    ...
    let run = run_hekma_agent(&["agent", "show", "alpha"], &ctx.project_dir, state_dir);
...
// ---- Story 6-3: the UJ-1 governance journey through documented hekma commands ----
...
fn uj1_governance_journey_through_documented_cli_commands() {
    ...
    let reg2 = run_hekma_agent(
        &["agent", "register", "free", "--kind", "hermes"],
        &ctx.project_dir,
        state_dir,
    );
    assert!(reg2.success, "register free: stderr={}", reg2.stderr);
    let ufree = run_hekma_agent(&["agent", "usage", "free"], &ctx.project_dir, state_dir);
...
// ---- Story 7-1: the UJ-3 journey through documented hekma commands ----
...
/// `hekma_conformance::uj3` poller), reads the LIVE Fleet row and asserts the
...
/// documented `hekma agent stop <name> --timeout 0` unless disarmed — best-effort,
...
fn uj3_governance_journey_through_documented_cli_commands_unix() {
    ...
    // library host test (`crates/hekma-engine/tests/uj3_library_host.rs`)
    // drives through the engine's `Blocking` facade is driven here through
    // DOCUMENTED `hekma` commands only — register --manifest, config set,
    // config get, show --json / usage --json, stop — and EVERY assertion comes
    // from the SHARED `hekma_conformance::uj3` module (the expectations are
    // pinned once; neither suite re-states them). The two suites drive
    // DIFFERENT roots (this one: the `KTESIO_STATE_DIR`-pinned hekma harness home;
    ...
    //
    // SINGLE-LIFETIME CLI BOUNDARY (the documented `hekma agent start` contract):
    // a standalone `hekma agent start` kills the process on the command's clean
    // engine drop, and ANY intervening `hekma` command that adopts a live process
...
/// Run the documented `hekma agent show <name> --json` and parse the instance row
...
fn show_entry(ctx: &TestContext, state_dir: &Path, name: &str) -> FleetEntry {
    ...
    assert_eq!(
        doc["schema_version"],
        serde_json::json!(hekma_engine::FLEET_SCHEMA_VERSION),
...
/// Run the documented `hekma agent usage <name> --json` and parse the usage
...
fn rate_and_cap_render_labeled_dollars_in_list_json_and_human() {
    ...
    let json = run_hekma_agent(&["agent", "list", "--json"], &ctx.project_dir, state_dir);
    assert!(
        json.success,
        "list --json should exit 0; stderr={}",
        json.stderr
    ...
    let human = run_hekma_agent(&["agent", "list"], &ctx.project_dir, state_dir);
...
fn list_budget_dollar_label_lives_in_the_header_not_the_truncatable_cell() {
    ...
    let human = run_hekma_agent_with_env(
        &["agent", "list"],
        &ctx.project_dir,
        state_dir,
        &[("COLUMNS", "116")],
...
fn show_of_a_rated_instance_surfaces_a_labeled_cost_row() {
    ...
    let show = run_hekma_agent(&["agent", "show", "priced"], &ctx.project_dir, state_dir);
    assert!(show.success, "show should exit 0; stderr={}", show.stderr);
    assert!(
...
fn show_of_a_no_rate_instance_says_dollar_features_are_inert() {
    ...
    let show = run_hekma_agent(&["agent", "show", "norate"], &ctx.project_dir, state_dir);
    assert!(show.success, "show should exit 0; stderr={}", show.stderr);
    assert!(
...
fn list_json_carries_a_fleet_totals_object_bumped_to_schema_2() {
    // Story 3-5 (AC-A/AC9): `hekma agent list --json` carries a top-level `totals` object
...
fn human_list_renders_the_fleet_total_footer() {
    // AC-A/AC-B: the human `hekma agent list` renders a Fleet-wide total footer on stdout.
...
// ---- Story 2-1: `hekma agent config set` / `get` (AC10, AC-B, AC7, AD-12) ----
...
fn config_get_persists_effective_config_snapshot_at_start() {
    ...
    // reads the Agent Home path from `register`'s stdout (path authority — hekma
    ...
    let start = run_hekma_agent(&["agent", "start", "snapcli"], &ctx.project_dir, state_dir);
    assert!(
        start.success,
...
/// A `hekma` run with ONE extra environment variable set on the child (so the env
/// SecretResolver can resolve `secret:NAME` at start). Mirrors `run_hekma_agent` but
/// adds `env(key, value)`; the child `hekma` process inherits it, and the engine it
/// spawns reads it via `std::env::var`. Local to the secret tests (the shared
/// helper does not need a general env knob).
fn run_hekma_agent_env(
    args: &[&str],
    working_dir: &Path,
    state_dir: &Path,
    env_key: &str,
    ...
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_hekma"))
        .args(args)
        .current_dir(working_dir)
        .env("KTESIO_NO_UPDATE_CHECK", "1")
        .env("KTESIO_STATE_DIR", state_dir)
        .env(env_key, env_val)
        .output()
        .expect("Failed to execute hekma");
    (
...
fn secret_reaches_the_adapter_but_never_leaks_and_reveal_shows_it() {
    ...
    // `hekma agent start`'s agent — a process that macOS CI never let become
    // observable and that Windows' kill-on-close reaps the instant `hekma`
    ...
    let (rok, rout, rerr) = run_hekma_agent_env(
...
fn secret_single_key_reveal_shows_only_that_leaf_and_default_masks() {
    ...
    let (rok, rout, _e) = run_hekma_agent_env(
        &[
            "agent",
            "config",
            "get",
    ...
    let (pok, pout, _e) = run_hekma_agent_env(
        &[
            "agent",
            "config",
            "get",
...
fn unresolved_secret_rejects_the_start_with_a_diagnostic_and_no_state_change() {
    ...
    let list = run_hekma_agent(&["agent", "list", "--json"], &ctx.project_dir, state_dir);
...
fn an_unbudgeted_instance_shows_the_honest_absent_budget() {
    ...
    // dollar Cost Cap.)
    let human = run_hekma_agent(&["agent", "show", "bare"], &ctx.project_dir, state_dir);
    assert!(
...
fn a_malformed_budget_value_is_rejected_at_write_time() {
    ...
    let bad_action = run_hekma_agent(
        &[
            "agent",
            "config",
            "set",
...
// ---- Story 4-2: `hekma agent logs <name> [--follow]` (AC-A, AC-B, AC-H) ----
...
fn logs_reads_retained_output_after_the_instance_stops_unix() {
    ...
    // it via a SEPARATE `hekma agent stop` invocation (which must first adopt
    // the still-live orphan), then a THIRD, separate `hekma agent logs`
    ...
    let stopped = run_hekma_agent(&["agent", "stop", "svc"], &ctx.project_dir, state_dir);
    assert!(
        stopped.success,
    ...
    let logs = run_hekma_agent(&["agent", "logs", "svc"], &ctx.project_dir, state_dir);
...
fn logs_follow_on_an_adopted_instance_reads_history_and_exits_cleanly_on_stop_unix() {
    ...
    // (`InteractionUnavailable`); HERE, `hekma agent logs --follow` reads the
    ...
    // with AC-C (a clean, non-hanging exit) through the real `hekma` binary.
    //
    ...
    // (`hekma-engine/tests/logs.rs`), not "...streams_new_output". Verified
    ...
    // Spawn `hekma agent logs svc --follow` in the BACKGROUND (not
    // `run_hekma_agent`, which blocks for full completion) so the main thread
    ...
    let mut child = std::process::Command::new(env!("CARGO_BIN_EXE_hekma"))
        ...
        .stderr(std::process::Stdio::piped())
        .spawn()
        .expect("spawn hekma agent logs --follow");
    ...
    let status = loop {
        ...
        assert!(
            std::time::Instant::now() < deadline,
            "hekma agent logs --follow must not hang after the instance stops"
        );
        std::thread::sleep(std::time::Duration::from_millis(50));
    };
    assert!(
...
fn logs_on_a_never_started_instance_is_empty_not_an_error() {
    ...
    let run = run_hekma_agent(&["agent", "logs", "nat"], &ctx.project_dir, state_dir);
    assert!(
        run.success,
...
fn logs_on_an_unregistered_name_is_not_found() {
    ...
    let state = TestContext::new();
    let state_dir = state.project_dir.as_path();
    let run = run_hekma_agent(&["agent", "logs", "ghost"], &ctx.project_dir, state_dir);
    assert!(
        !run.success,
...
// ===========================================================================
//
// These are THE compatibility gate for `hekma`'s two machine-facing contracts:
...
// by design — it compares the `hekma-engine` *Rust* public API. It does NOT
...
// It also never inspects the `hekma` binary crate, where the CLI-local documents
...
fn usage_json_totals_equal_the_list_json_totals_exactly() {
    ...
    let state_dir = state.project_dir.as_path();
    let list = run_hekma_agent(&["agent", "list", "--json"], &ctx.project_dir, state_dir);
    let usage = run_hekma_agent(&["agent", "usage", "--json"], &ctx.project_dir, state_dir);
...
fn usage_on_an_empty_fleet_prints_guidance_on_stderr_and_stays_pure_json() {
    ...
    let human = run_hekma_agent(&["agent", "usage"], &ctx.project_dir, state_dir);
    ...
    let json = run_hekma_agent(&["agent", "usage", "--json"], &ctx.project_dir, state_dir);
...
fn show_and_usage_json_priced_shape_key_sets_are_frozen() {
    ...
    let fleet = run_hekma_agent(&["agent", "usage", "--json"], &ctx.project_dir, state_dir);
...
fn register_incompatible_contract_manifest_exits_one_naming_both_versions_and_the_rule() {
    // FR-30 end-to-end at the REAL `hekma` binary (the independent pass found the
    // hekma-side negotiation proof was unit-only): registering a manifest whose
    ...
    let show = run_hekma_agent(&["agent", "show", "m"], &ctx.project_dir, state_dir);
...
/// `hekma agent logs` reads back — `<state>/agents/<name>/logs/output.log`, JSON
...
fn append_captured_log_lines(state_dir: &Path, name: &str, lines: &[hekma_engine::LogLine]) {
    use std::io::Write;
...
fn captured(name: &str, stream: hekma_engine::LogStream, text: &str) -> hekma_engine::LogLine {
    hekma_engine::LogLine::new(name, stream, text, "2026-07-20T12:00:00Z")
}
...
fn logs_json_wire_shape_is_frozen_ndjson_on_every_os() {
    ...
    append_captured_log_lines(
        ...
        "nat",
        &[
            captured("nat", hekma_engine::LogStream::AgentOut, "first out"),
            captured("nat", hekma_engine::LogStream::AgentErr, "second \"err\""),
            captured("nat", hekma_engine::LogStream::Engine, "third engine"),
        ],
    );
    ...
    let human = run_hekma_agent(&["agent", "logs", "nat"], &ctx.project_dir, state_dir);
...
fn logs_follow_json_emits_an_incremental_batch_as_valid_ndjson() {
    ...
    let status = loop {
        ...
        assert!(
            ...
            "hekma agent logs --follow --json must not hang on a non-running instance"
        );
        std::thread::sleep(std::time::Duration::from_millis(20));
    };
    assert_eq!(
...
fn logs_json_survives_a_consumer_that_stops_reading_and_still_exits_zero() {
    ...
    // documents ("Every `hekma` command returns one of these numeric exit codes").
    ...
    let (ctx, state) = registered_mock("nat");
    let state_dir = state.project_dir.as_path();
    let lines: Vec<hekma_engine::LogLine> = (0..5_000)
        .map(|i| {
            captured(
                "nat",
                hekma_engine::LogStream::AgentOut,
    ...
    for json in [true, false] {
        ...
        let status = child.wait().expect("wait for hekma agent logs");
        assert_eq!(
            status.code(),
            Some(0),
...
fn logs_json_emits_newline_delimited_self_versioned_loglines_in_append_order_unix() {
    ...
    let human = run_hekma_agent(&["agent", "logs", "svc"], &ctx.project_dir, state_dir);
    let human_texts: Vec<String> = human
        .stdout
        .lines()
        .filter(|l| !l.trim().is_empty())
...
// `crates/hekma/src/exit_code.rs`. The tests below prove the OTHER half: that the
...
fn agent_show_missing_instance_exits_with_the_not_found_code() {
    ...
    for args in [
        ...
        let run = run_hekma_agent(&args, &ctx.project_dir, state_dir);
        assert_eq!(
            run.code,
            Some(3),
...
// ---- Story 5-1: `hekma agent memory attach | detach` ----
//
