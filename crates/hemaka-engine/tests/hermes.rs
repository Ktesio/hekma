...
fn hermes_registers_with_its_declared_per_os_shape_and_self_reported_metering() {
    ...
    let pause = caps
        .entries
        .iter()
        .find(|(c, _)| *c == hemaka_engine::Capability::Pause)
        .expect("pause is declared");
    assert_eq!(
        pause.1,
        hemaka_engine::SupportLevel::BestEffort,
        "Hermes pause must be BEST-EFFORT on every OS (CP-a)"
    );
    let interaction = caps
        .entries
        .iter()
        .find(|(c, _)| *c == hemaka_engine::Capability::Interaction)
        .expect("interaction is declared");
    assert_eq!(
        interaction.1,
        hemaka_engine::SupportLevel::Guaranteed,
    ...
    let launch = hemaka_engine::adapter::native_launch(hemaka_adapters_hermes::HERMES_KIND)
        .expect("declared");
...
fn hermes_memory_composition_maps_the_managed_dir_onto_hermes_home_exactly_as_start_would() {
    ...
    let mapping = hemaka_engine::adapter::resolve_config_mapping("hermes", None).unwrap();
    let env_var = mapping
    ...
    let mut launch = hemaka_engine::adapter::StartLaunch {
        exec: hemaka_adapters_hermes::HERMES_KIND.to_string(),
        args: vec![
            "gateway".to_string(),
            "run".to_string(),
    ...
    hemaka_engine::adapter::apply_config_mapping(
        &mut launch,
        &mapping,
        &effective,
        &BTreeMap::new(),
...
/// its script target via [`hemaka_conformance::fake_agent_bin`], which anchors
...
fn install_shim(shim_dir: &TempDir) -> PathBuf {
    ...
    let source = if candidate.exists() {
        ...
        let status = Command::new(cargo)
            .args(["build", "-p", "hemaka-conformance", "--bin", "hermes_shim"])
    ...
    let agent = hemaka_conformance::fake_agent_bin();
    std::fs::copy(
        &agent,
        shim_dir
            .path()
