...
//!    SHARED `hemaka_conformance::uj3` expectations (same expectations,
...
use hemaka_engine::{broadcast, AdapterRef, ConfigLayer, Engine, EngineEvent, LifecycleState};
...
struct FlowResult {
    ...
    /// [`EVENT_BUS_CAPACITY`](hemaka_engine::EVENT_BUS_CAPACITY)).
    lagged: Option<u64>,
}
...
fn two_engines_in_one_process_drive_the_full_uj3_flow_concurrently_without_collision() {
    ...
    let _warm = hemaka_conformance::fake_agent_bin();
...
fn assert_subscriber_isolation(result: &FlowResult) {
    ...
    let received: Vec<&hemaka_engine::TransitionEvent> = result
        .events
        .iter()
    ...
    let received_breaches: Vec<&hemaka_engine::BudgetBreachEvent> = result
        .events
        .iter()
        .filter_map(|e| match e {
...
fn instance_row_count(root: &Path) -> u64 {
    let conn = rusqlite::Connection::open(root.join(hemaka_engine::paths::STATE_DB_FILE))
...
fn fleet_entry(facade: &hemaka_engine::Blocking<'_>) -> hemaka_engine::FleetEntry {
    facade
        .fleet()
        .expect("fleet read")
        .into_iter()
