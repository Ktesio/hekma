...
fn resync_returns_exactly_the_committed_logs_after_the_crash_window() {
    ...
    let batch = facade
        .resync_events(uj3::FLOW_INSTANCE, hemaka_engine::ResyncCursor::START)
...
fn a_resync_cursor_continues_from_where_the_host_left_off() {
    ...
    let first = facade
        .resync_events("resync-cursor", hemaka_engine::ResyncCursor::START)
...
fn resync_surfaces_the_expected_error_arms_and_tolerates_a_torn_tail() {
    ...
    assert!(matches!(
        facade
            .resync_events("Bad Name", hemaka_engine::ResyncCursor::START)
            .unwrap_err(),
    ...
    assert!(matches!(
        facade
            .resync_events("never-registered", hemaka_engine::ResyncCursor::START)
            .unwrap_err(),
    ...
    let batch = facade
        .resync_events("resync-edges", hemaka_engine::ResyncCursor::START)
