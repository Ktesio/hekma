...
use hemaka_engine::{AdapterRef, BreachDimension, BreachScope, Engine, LifecycleState};
...
fn fleet_cumulative_dollars(facade: &hemaka_engine::Blocking, name: &str) -> i64 {
    facade
        .fleet()
        .unwrap()
        .iter()
...
fn set_unit_rate(facade: &hemaka_engine::Blocking, name: &str) {
...
fn a_dollar_cap_breach_pauses_by_default_and_records_a_dollar_breach() {
    ...
    assert!(
        matches!(
            &paused.cause,
            hemaka_engine::TransitionCause::BudgetExceeded {
