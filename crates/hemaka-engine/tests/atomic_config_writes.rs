...
//! gate's integration-test allowlist (`crates/hemaka-engine/tests/`, the AI-35
...
use hemaka_adapter_api::{ConfigMapping, ConfigTarget};
use hemaka_engine::paths::write_atomically;
use hemaka_engine::{ConfigLayer, Engine, SourceLayer};
...
/// An [`hemaka_engine::EffectiveConfig`] whose single INSTANCE-layer leaf is
...
fn effective_with_model(value: &str) -> hemaka_engine::EffectiveConfig {
    let layers = [
        ConfigLayer::empty(),
        ConfigLayer::empty(),
        ConfigLayer::parse(
    ...
    hemaka_engine::domain::resolve(layers)
}
...
fn apply_file_target_failed_write_leaves_the_previous_native_file_unchanged() {
    ...
    let err = hemaka_engine::adapter::apply_config_mapping(
        &mut launch,
        &mapping,
        &second,
        &BTreeMap::new(),
    ...
    assert!(
        matches!(
            err,
            hemaka_engine::adapter::ConfigApplyError::FileRender { .. }
        ),
