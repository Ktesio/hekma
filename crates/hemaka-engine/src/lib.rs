...
//!   `hemaka-adapter-api` so `kt` can render the effective per-OS declaration.
...
//! `hemaka-adapter-api` types); this crate depends on `hemaka-adapter-api` and
