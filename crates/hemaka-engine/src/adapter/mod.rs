...
//!
//! The loader reads, parses (via [`hemaka_adapter_api::Manifest`]), and
...
//! `hemaka-adapter-api`'s parsed [`Manifest`]/[`CapabilityDeclaration`]/
...
//! [`builtin::BuiltinMock`], deliberately NOT the `hemaka-conformance`
...
pub enum AdapterResolveError {
    ...
    /// [`hemaka_adapter_api::ContractVersionError`], so the rule text lives in
...
/// `[lifecycle.start]` [`OpTemplate`](hemaka_adapter_api::OpTemplate) or a native
...
pub enum LaunchResolveError {
    ...
    /// [`hemaka_adapter_api::ContractVersionError`]).
    #[error("adapter.toml at {path} is incompatible: {detail}")]
    ContractIncompatible {
        /// The manifest path.
        path: String,
...
///   (reusing [`hemaka_adapter_api::Manifest`] — the same parser registration
...
pub fn resolve_start_launch(
    ...
    let manifest_version = manifest
        ...
        .unwrap_or_default()
        .trim();
    if let Err(negotiation) = hemaka_adapter_api::negotiate_contract_version(manifest_version) {
        ...
        for version in ["2.1.0", "2.0.0-rc.1", "2.0.0+build.9"] {
            ...
            match err {
                AdapterResolveError::ContractIncompatible { path, detail } => {
                    ...
                    assert!(
                        detail.contains(hemaka_adapter_api::CONTRACT_VERSION),
                        "names the engine version: {detail}"
                    );
                    assert!(
                        detail.contains(hemaka_adapter_api::COMPATIBILITY_RULE),
...
"#;
    ...
    fn enforce_invariants_rejects_all_unsupported_native_declaration() {
        ...
        use hemaka_adapter_api::{Capability, SupportLevel};
...
"#;
    ...
    fn resolve_start_launch_resolves_the_hermes_builtin_launch() {
        ...
        let launch = resolve_start_launch(hemaka_adapters_hermes::HERMES_KIND, None)
            .expect("hermes carries a launch");
        assert_eq!(launch.exec, hemaka_adapters_hermes::HERMES_KIND);
...
"#;
        let path = write_manifest(tmp.path(), body);
        let launch = resolve_start_launch(hemaka_adapters_hermes::HERMES_KIND, Some(&path))
...
"#;
    ...
    fn apply_delivers_resolved_cleartext_for_a_secret_leaf_not_the_mask() {
        ...
        assert_ne!(
            launch.env.get("MODEL").map(String::as_str),
            Some(hemaka_adapter_api::OsId::current().as_str()), // arbitrary non-equal
        );
        assert_ne!(
            launch.env.get("MODEL").map(String::as_str),
    ...
    fn apply_secret_leaf_missing_from_map_fails_closed_to_the_mask() {
        ...
        assert_eq!(
            launch.env.get("MODEL").map(String::as_str),
            Some(hemaka_engine_secret_mask()),
    ...
    fn hemaka_engine_secret_mask() -> &'static str {
...
"#;
    ...
    // `crates/hemaka-engine/tests/atomic_config_writes.rs`
