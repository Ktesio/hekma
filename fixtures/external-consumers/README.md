...
Out-of-tree consumers proving the Hemaka rename (v0.8.0) is surface-safe
...
- `engine-consumer/` — the embedding-facade lifecycle (open → register →
  ...
  `ktesio-engine` (freeze commit 49da96b) and HEAD's `hemaka-engine` —
...
- `both-graph/` — depends on the PUBLISHED `ktesio-engine` 0.3.0 AND
  `hemaka-engine` in ONE dependency graph, proving a host mid-migration
  keeps building. Armed by CI once `hemaka-engine` is on crates.io
