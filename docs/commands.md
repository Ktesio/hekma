...
Two fields are **additive and `acp`-specific** (story 14-3; every other kind omits them): `acp_context_usage` — the agent's reported session context window (`used`/`size` plus its optional self-reported cost), a context-grain figure that is never part of the billing `usage` object and is present only while a live ACP connection has reported one; and `usage_gap` — present only for an `acp` instance whose Usage Ledger holds no billing-grade usage at all, carrying the tiers attempted (`observed`/`sentinel`), whether context usage was reported, and a one-line `notice` naming all three.
