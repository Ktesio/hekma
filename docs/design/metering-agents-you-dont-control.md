...
Those token totals are the ledger sums exactly, the same numbers `kt agent list` and `kt agent show` print. With no rate configured the dollar fields are absent rather than `0`. The code is [source-available](https://github.com/Ktesio/ktesio), so every sentence in this section can be checked against it.
...
An `https://` upstream works: the proxy dials it directly over a vendored rustls+ring stack — no system TLS library, the same vendored philosophy as the bundled SQLite, and one rustls+ring pair shared with `kt`'s own download client. The trust roots are vendored Mozilla roots (webpki-roots), not your OS keychain, so a provider behind a private CA your machine trusts will still fail the handshake with an honest 502.
