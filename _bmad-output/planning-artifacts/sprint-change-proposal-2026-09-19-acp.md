...
| 14-3 | **Honest fleet surfaces** — metering cells show the honest `—` (D2); docs state ACP agents are unmetered in v1; config mapping (env/args) and Memory Backing delivery work unchanged through existing machinery; `--help`/notice honesty per the house pattern. |
...
**D2 — Metering: honest unmetered in v1.** ACP v1 carries no usage data, and third-party ACP agents cannot be assumed to emit the KTESIO_USAGE sentinel (their stdout is the protocol channel; their stderr is the shared agent.log but nothing writes the sentinel there). Fleet/config surfaces show the honest `—` (the `FleetEntry::METERING_SEED_CELL` pattern), docs state it plainly, and no estimate is fabricated (house estimate-honesty rule, AD-8). Revisit triggers: ACP gains a usage surface, or a specific agent gets a bespoke adapter with self-reporting.
...
**Success criteria.** `hekma agent register <name> --kind acp` + a launch command starts any ACP v1 agent under full supervision; lifecycle/config/memory/fleet behave identically (SM-1's bar) with the honest `—` metering; detach→adopt resumes the ACP session when the agent supports it; all four house gates green; docs current (NFR-7); the frozen v1 contracts show no diff (semver clean).

---

## Approval

**APPROVED by Islam 2026-09-19** (batch mode): the proposal as a whole and decisions D1–D6 ratified explicitly via the correct-course approval gate. **Amendment ratified same session:** D7 added (Hermes speaks ACP — the bespoke `hermes` kind deprecates via story 14-5, removal at a later major); D6 refined to a version RANGE (Hermes tracks 0.8.1–<1.0). Epic 14 = stories 14-1…14-5. Consequence: Epic 14 + stories 14-1…14-4 added to `epics.md` and sprint-status (backlog); D2–D6 carry into the epic's spec stage as ratified constraints; AD-19's spine text lands at 14-1's spec stage from the seed in Section 4.
