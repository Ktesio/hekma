...
def main() -> int:
    ...
    if args.update_files:
        upsert_release_section(
            ROOT / "CHANGELOG.md",
            "# Changelog\n\nAll notable changes to Hekma (formerly Ktesio) are generated from git history.\n",
            args.tag,
            section,
        )
        upsert_release_section(
...
def render_release_body(tag: str, previous_tag: str | None, commits: list[Commit]) -> str:
    ...
    # v0.8.0+ (the Hekma rename): every release body points kt users at
...
def render_asset_table(tag: str) -> list[str]:
    ...
    for platform, target, extension in TARGETS:
        archive = f"hekma-{tag}-{target}.{extension}"
    ...
    aggregate = f"https://github.com/{REPO}/releases/download/{tag}/hekma-{tag}-checksums.txt"
    lines.append(f"| All | checksums | [hekma-{tag}-checksums.txt]({aggregate}) | - |")
    return lines
