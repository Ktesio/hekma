...
#     HEKMA_INSTALL_* aliases are preferred (either is accepted).
...
# forward-looking HEKMA_* name wins; the legacy KTESIO_* name is the
...
METHOD="$(first_env auto HEKMA_INSTALL_METHOD KTESIO_INSTALL_METHOD)"
...
is_dry_run() {
  dry="$(first_env "" HEKMA_INSTALL_DRY_RUN KTESIO_INSTALL_DRY_RUN)"
  is_truthy "$dry"
}
...
# The existing install to migrate: `hekma` first, then the retired
...
# `hekma --version` (and `hkm --version` — the alias reports the shared
# identity) prints "hekma <version>".
is_hekma_binary() {
  output=$("$1" --version 2>/dev/null || true)
  case "$output" in
    "hekma "[0-9]* | "hekma v"[0-9]*)
      return 0
      ;;
    *)
      return 1
...
is_owned_binary() {
  is_hekma_binary "$1" || is_legacy_kt_binary "$1"
}
...
brew_has_hekma() {
  ...
  brew list --formula hekma >/dev/null 2>&1 ||
...
detect_existing_method() {
  ...
  case "$existing_path" in
    */Cellar/hekma/* | */Cellar/ktesio/*)
      say "brew"
      return 0
      ;;
  esac

  if brew_has_hekma; then
    say "brew"
    return 0
  fi
...
default_install_dir() {
  ...
  fail "HEKMA_INSTALL_DIR (or KTESIO_INSTALL_DIR) is required when HOME is not set."
}
...
detect_release_target() {
  ...
  case "$os_name:$arch_name" in
    ...
    Linux:x86_64 | Linux:amd64)
      ...
      ;;
    *)
      fail "No prebuilt Hekma binary is available for ${os_name}/${arch_name}. Install Rust and run: cargo install hekma --force"
      ;;
  esac
}
...
note_retired_kt() {
  ...
  if [ -e "$retired_path" ] && is_legacy_kt_binary "$retired_path"; then
    warn "the retired kt binary was left at $retired_path — Hekma 0.8.0 replaced it with hekma + hkm; remove it with: rm $retired_path"
  fi
}
...
prepare_binary_target() {
  ...
  install_dir="$(first_env "" HEKMA_INSTALL_DIR KTESIO_INSTALL_DIR)"
  ...
  if [ -d "$install_dir" ] && [ ! -w "$install_dir" ]; then
    fail "$install_dir is not writable. Set HEKMA_INSTALL_DIR (or KTESIO_INSTALL_DIR) to a writable directory on PATH."
  fi

  for candidate in "$BIN" "$HKM"; do
...
install_with_binary() {
  ...
  if is_dry_run; then
    say "DRY RUN: install prebuilt $target ($BIN + $HKM) to $install_dir"
    if ! dir_is_on_path "$install_dir"; then
      warn "$install_dir is not on PATH. Add it before running hekma."
    fi
    return 0
  fi

  tag=$(latest_release_tag)
  asset="hekma-${tag}-${target}.tar.gz"
  asset_url="${RELEASE_BASE_URL}/${tag}/${asset}"

  tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/hekma-install.XXXXXX")
  trap 'rm -rf "$tmpdir"' EXIT HUP INT TERM
  package_dir="$tmpdir/package"
  mkdir -p "$package_dir"

  say "Downloading Hekma ${tag} for ${target}..."
  ...
  say "Installed Hekma to $install_dir/$BIN and $install_dir/$HKM"
...
main() {
  case "$METHOD" in
    ...
    brew | cargo | binary)
      ;;
    *)
      fail "HEKMA_INSTALL_METHOD (or KTESIO_INSTALL_METHOD) must be one of: auto, brew, cargo, binary."
      ;;
  esac
